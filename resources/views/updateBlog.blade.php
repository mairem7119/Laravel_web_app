    ...
    <form action="{{route('createBlog')}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="cover-blog">
            ...
            <div class="create-blog-title">
                <input class="input-group" name="title" type="text" placeholder="Your blog title" value="{{$blog->title}}">
                @error('title')
            ...
            <div class='create-blog-content'> 
                <textarea placeholder="Enter some text..." name="content">{{$blog->content}}</textarea> 
                 @error('content') 
                    <div class="error">{{ $message }}</div>
                 @enderror 
            </div>
